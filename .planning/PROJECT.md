...
Validated in v1.0 Phase 5 (slidev-integration): SLIDES-01/02/04/06 + CONTENT-04 — `vedmichv/slidev:gh-pages` added as submodule at `slidev/` (pinned 1dfa2ec0, `branch = gh-pages` load-bearing per Pitfall 4); `.github/workflows/deploy.yml` extended with `submodules: recursive` + commented-out whitelist `cp` step (empty per D-12 — no decks migrate this phase), 2-job topology preserved; `PresentationCard` + `search-index` emit `/slides/<slug>/` internal URLs with `data.slides` frontmatter override (D-16); homepage Presentations subtitle drops `s.vedmich.dev` (bilingual); all 12 presentation MDX flipped to `draft: true` with full frontmatter skeleton preserved (D-03 + D-11); bilingual empty-state `Decks coming soon...` / `Слайды появятся скоро...` ships for `totalCount === 0`; `docs/slides-onboarding.md` canonical runbook (240 LOC, 10 H2 sections — When-to-use + Prerequisites + 6 numbered Steps + Gotchas + Further reading); `~/.claude/skills/vv-slidev/references/publish-to-vedmich-dev.md` short TL;DR pointer (46 LOC per D-10) with byte-identical vault mirror at `~/Documents/ViktorVedmich/80-Resources/85-Scripts/85.20-Claude-Code/skills/vv-slidev/references/` (separate vault commit 8d88cfc per three-way-sync rule); `## Slidev Integration — INFRASTRUCTURE READY (since 2026-05-07)` H2 block in CLAUDE.md + stale `s.vedmich.dev` references swept; SLIDES-03 (deck migration) + SLIDES-05 (s.vedmich.dev 301 redirect) deferred per D-17 with explicit rationale; live CDN deploy success on run 25519821890 (build 28s — well under +5s D-07 budget over 90s baseline; submodule checkout + empty whitelist negligible); 11 code-review findings (3 CR / 5 WR / 3 IN) all latent and non-blocking (hidden by universal `draft: true` + commented-out whitelist) tracked for un-draft day. 4 pre-existing Shiki visual regression failures confirmed failing at c703400 baseline — not Phase 5 regression.

Validated in v1.0 Phase 4 (excalidraw-pipeline): DIAG-01..05 — `scripts/excalidraw-to-svg.mjs` (178 LOC) using `@aldinokemal2104/excalidraw-to-svg@1.1.1` (exact-pinned devDep per D-01d/D-01e); SVGO `preset-default` + `removeDesc: false` + `multipass: true` with pre-SVGO `<title>`+`<desc>` injection via `escapeXml`; 10 KB budget gate + `validatePath` (REPO_ROOT ∪ os.tmpdir) path-traversal guard + `validateFilesBlob` 100 KB cap; `public/blog-assets/<slug>/diagrams/*.svg` convention; MCP post ASCII `←→` diagram replaced with bilingual `<img>` embed of 9,673 B client-server SVG (EN + RU); karpenter post gains 7,338 B split-ownership SVG (EN + RU) after the "split ownership" paragraph (Helvetica fontFamily=2 to fit budget per D-04b Deviations §1); `diagrams-source/README.md` contributor runbook (118 LOC, 6 H2 sections); `visuals-routing.md` Priority 1 `excalidraw` row extended with concrete delivery path; REQUIREMENTS.md DIAG-05 language reconciled per D-06-revised (DIAG-01 unchanged per D-01e); 9 new integration tests (44 total at phase close), all green; boundary invariant holds (`@aldinokemal2104` / `@excalidraw` never appear in `src/` or `dist/`). Tech debt BL-01 (symlink-bypass on validatePath), BL-02 (regex-based title/desc injection not XML-aware), and WR-04 (Helvetica OS-dependent) all remediated in Phase 04.1 (excalidraw-pipeline-hardening, 58-finding review sweep): realpath walk + ALLOWED_WRITE_ROOTS, explicit `removeScripts` + `removeAttributesBySelector`, dataURL scheme whitelist, parsererror detection, white-rect SVGO strip, Helvetica fallback chain, figure/figcaption MDX wrap, MCP re-authored with Excalifont 28px labels (canvas aspect reshape 820×300 deferred with rationale), 37/58 findings closed + 21 deferred across P1-P4 tiers.
...
**Last updated:** 2026-05-07 (Phase 5 slidev-integration shipped 2026-05-07 — SLIDES-01/02/04/06 + CONTENT-04 shipped; SLIDES-03 + SLIDES-05 deferred per D-17; 4 plans across 3 waves, infrastructure-and-contract-only per D-01/D-12; submodule at slidev/ pinned 1dfa2ec0; runbook docs/slides-onboarding.md 240 LOC; CDN smoke + D-07 budget confirmed live on run 25519821890)
